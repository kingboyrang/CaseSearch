...
-(void)asynService:(ServiceArgs*)args progress:(progressRequestBlock)progress completed:(blockFinished)finish failed:(blockFailed)failed{
        ...
        _progressBlock=Block_copy(progress);
         ...
         Block_release(_blockFinished);
        _blockFinished=Block_copy(finish);
   
        Block_release(_blockFailed);
         _blockFailed=Block_copy(failed);
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(finishedRequest:)]) {
        [self.delegate finishedRequest:result];
    }

	if(_blockFinished){
        _blockFinished(result);
    }
}
...
-(NSString*)soapMessageResult:(ASIHTTPRequest*)request{
    ...
    //[SoapXmlParseHelper SoapMessageResultXml:responseString ServiceMethodName:methodName];

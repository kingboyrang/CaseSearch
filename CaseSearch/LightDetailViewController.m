...
-(void)handlerXml:(NSString*)xml{
    ...
    CircularLight *road=[SoapXmlParseHelper RootNodeToObject:xml className:@"CircularLight"];
    ...
    if ([road.Location length]>0) {
        [self autolabelSize:self.cellAddress withText:road.Location];
        self.cellLightNumber.hidden=YES;
        self.cellLightNumber.frame=CGRectMake(0, 0, 0, 0);
    }else{
        [self autolabelSize:self.cellLightNumber withText:road.LightNumber];
        self.cellAddress.hidden=YES;
        self.cellAddress.frame=CGRectMake(0, 0, 0, 0);
    }
